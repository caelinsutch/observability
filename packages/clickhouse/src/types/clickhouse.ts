...
export interface EventRecord
	extends Omit<Event, "attributes" | "resource_attributes"> {
...
export function parseEventRecord(record: EventRecord): Event {
	return {
		...record,
		attributes: JSON.parse(record.attributes) as Record<string, unknown>,
		resource_attributes: JSON.parse(record.resource_attributes) as Record<
			string,
			unknown
		>,
	} as Event;
...
export function prepareEventForInsert(event: Event): EventRecord {
	return {
		...
		resource_attributes:
			typeof event.resource_attributes === "string"
